...
class NaiveBayes:
    ...
    def update_model(self, bow, label):
        ...
        for key, values in bow.items():
            if key not in self.class_word_counts[label]:
                self.class_word_counts[label][key] = 0.0
            self.class_word_counts[label][key] += values
            self.class_total_doc_counts[label] += 1
            self.class_total_word_counts[label] += values
            self.vocab.add(key)
    ...
    def p_word_given_label(self, word, label):
        ...
        return float(self.class_word_counts[label][word])/float(self.class_total_word_counts[label])
    ...
    def p_word_given_label_and_pseudocount(self, word, label, alpha):
        ...
        We just assume we saw each word one more time that is sum of all words + len(wordcounts)
        """
        return float(self.class_word_counts[label][word] + alpha)/float((self.class_total_word_counts[label]) + alpha*len(self.vocab))
    ...
    def log_likelihood(self, bow, label, alpha):
        ...
        word_list = []
        for word in bow:
            word_list.append(self.p_word_given_label_and_pseudocount(word, label, alpha))
        log_word_list = map(lambda w:math.log(w), word_list)
        return math.fsum(log_word_list)
    ...
    def log_prior(self, label):
        ...
        """
        return math.log(self.class_total_doc_counts[label]/math.fsum(self.class_total_doc_counts.values()))
    ...
    def classify(self, bow, alpha):
        ...
        class_prob = map(lambda label:self.unnormalized_log_posterior(bow, label, alpha), labels)
